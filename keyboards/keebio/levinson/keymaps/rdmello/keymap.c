...
/* COMMON ROW
 * ,------------------------------------------------------------------------------------------------.
 * |Adjust|Raise |Space | GUI  | LOWER| GUI(SPC)| |CTL(SPC) | RAISE| Alt  |      |      |TGL_CTL_GUI|
 * `------------------------------------------------------------------------------------------------'
 */
#define RYLAN_LAYOUT(...) LAYOUT_ortho_4x12( \
   __VA_ARGS__, \
   ADJUST, RAISE, KC_SPC, KC_LGUI, LOWER, LGUI_T(KC_SPC),      LCTL_T(KC_SPC), RAISE, KC_LALT, _______,  _______, MAGIC_TOGGLE_CTL_GUI \
)
...
[_RAISE] =  RYLAN_LAYOUT( \
  ...
  CTL_T(KC_ESC),  KC_MS_L, KC_MS_D, KC_MS_R,   KC_MS_WH_UP,  KC_END,     KC_LEFT, KC_DOWN,   KC_UP, KC_RGHT, KC_BSLS, KC_LOCK, \
